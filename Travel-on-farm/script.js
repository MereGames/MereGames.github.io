  ...
  if(windowFocus == true)
  {
	...
	if(type == "main" || type == "ratingMenu")
	{
		...
		if(buyPlayers[selectPlayer][0].buy == false) {
			selectPlayer++;
			if(selectPlayer == 1 && obj_system.moneyPlayer < 25) {
				ctx.globalAlpha = 0.5;
			}
			if(selectPlayer == 3 && obj_system.moneyPlayer < 50) {
				ctx.globalAlpha = 0.5;
			}
			selectPlayer--;
		}
	...
	else if(type == "game" || type == "gameOver" || type == "rating")
	{
			...
			if(viewTutorial == true)
		{
			...
			ctx.font = "30px Impact, fantasy";
		    ctx.fillStyle = "red";
		    ctx.fillText("Прыгать", 370, 340);
			ctx.fillText("Стрелять", 140, 400);
			ctx.restore();
			
			ctx.fillStyle = "#fff";
			ctx.font = "25px cursive";
			ctx.textAlign = "center";
			ctx.fillText("Нажмите на W, чтобы начать", canvas.width/2, 430);
		}
		}
  ...
  if(windowFocus == true && gamePosition != "preLoad")
  {
	...
	if(type == "main" || type == "ratingMenu")
	{
		...
		if(buyPlayers[0][0].buy == true) {
			...
			ctx.fillText("25", obj_border.x - 140, 357);
		}
		...
		if(buyPlayers[2][0].buy == true) {
			...
			ctx.fillText("50", obj_border.x + 300, 357);
		}
		...
		ctx.fillStyle = "red";
		ctx.font = "25px Brushstroke, fantasy";
		ctx.fillText("Лучшее расстояние:\n" + Math.round(obj_system.best_score) + "м", 300, 70);
...
function buyPlayersSystem() {
	if(selectPlayer == 1) {
		if(obj_system.moneyPlayer >= 25) {
			selectPlayer--;
			obj_system.moneyPlayer -= 25;
