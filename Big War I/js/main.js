...
//Constants:
const TILE_SIZE = 64;
const NUM_MENU = 2, NUM_BUTTONS = 9, NUM_GROUND = 10, NUM_BORDERS = 7, NUM_BUILDS = 12, NUM_ENEMY = 3, NUM_OBJS = 7, NUM_BOOM = 1;
...
var helpImgRu = new Image();
helpImgRu.src = "img/help_ru.png";
otherImages.push(helpImgRu);

var helpImgRu = new Image();
helpImgRu.src = "img/help_en.png";
otherImages.push(helpImgRu);
	    ...
	    if(gameConfig[0].position == "loading") {
		    //-----------------------------------------------------------------------
		        ...
		        if(gameConfig[0].leng == "en") {
		            ...
		            ctx.drawImage(otherImages[6], WIDTH/2 - 230, HEIGHT - 90);
		        }else if(gameConfig[0].leng == "ru"){
		        	ctx.fillText("Загрузка...", WIDTH/2 - 140, HEIGHT/2);
		        	ctx.drawImage(otherImages[5], WIDTH/2 - 230, HEIGHT - 90);
		        }
		        document.body.style.cursor = "default";
	    }
	...
	if(gameConfig[0].position != "logo" && gameConfig[0].pre_position == "none") {
		if(gameConfig[0].position == "menu") {
		    ...
		    for(let d = 0; d < buildings.length; d++) {
		    	...
		    	if(delBuilding.open == true && stopGame == false) {
		    		if(gameConfig[0].position == "free") {
		    		for(let i = 0; i < mapsGame[idMap].map.length; i++) {
		    			for(let j = 0; j < mapsGame[idMap].map.length; j++) {
		    				for(let w = 0; w < buildsGame.length; w++) {
		    			    if(x >= TILE_SIZE * i - movAddX && x <= TILE_SIZE * i - movAddX + TILE_SIZE &&  y >= TILE_SIZE * j - movAddY && y <= TILE_SIZE * j - movAddY + TILE_SIZE) {
								delBuilding.x = TILE_SIZE * i - movAddX;
								delBuilding.y = TILE_SIZE * j - movAddY;
						  }
		    		    }
		    		  }
		    		}
		    		...
		    		if(x >= buildsGame[num].x - buildsGame[num].radius - movAddX && x <= buildsGame[num].x - buildsGame[num].radius - movAddX + 64 && y >= buildsGame[num].y - buildsGame[num].radius - movAddY && y <= buildsGame[num].y - buildsGame[num].radius - movAddY + 64 && stopGame == false) {
				        if(gameConfig[0].position == "free" || gameConfig[0].position == "level") {
				        	if(buildsGame[num].map == idMap && gameConfig[0].position == "free") {
					            if(buildsGame[num].name == "army" || buildsGame[num].name == "armyHard" || buildsGame[num].name == "armyFast" || buildsGame[num].name == "armyTwo" || buildsGame[num].name == "armyHard2" || buildsGame[num].name == "armyFast2") {
...
function clickEvent(e) {
	...
	if(gameConfig[0].position != "logo" && gameConfig[0].pre_position == "none") {
		if(gameConfig[0].position == "menu") {
			...
			//Baze main
			if(x >= objBaze.x - objBaze.radius - movAddX && x <= objBaze.x - objBaze.radius - movAddX + 64 && y >= objBaze.y - objBaze.radius - movAddY && y <= objBaze.y - objBaze.radius - movAddY + 64 && stopGame == false && e.shiftKey == false && delBuilding.open == false) {
				if(gameConfig[0].position == "free") {
		    ...
		    for(let ar = 0; ar < buildsGame.length; ar++) {
		    	if(x >= buildsGame[ar].x - buildsGame[ar].radius - movAddX && x <= buildsGame[ar].x - buildsGame[ar].radius - movAddX + 64 && y >= buildsGame[ar].y - buildsGame[ar].radius - movAddY && y <= buildsGame[ar].y - buildsGame[ar].radius - movAddY + 64 && stopGame == false && e.shiftKey == false && delBuilding.open == false) {
			  ...
			  }else if(buildsGame[ar].name == "armyHard2") {
		    			if(gameConfig[0].position == "free") {
				if(mapsGame[idMap].playerData.money >= objsProp.player.tank_hard2.price) {
					buildsGame[ar].num += 1;
				    mapsGame[idMap].playerData.money -= objsProp.player.tank_hard2.price;
			    }
			    }else if(gameConfig[0].position == "level") {
			    	if(levelsMaps[select_level].map[0].playerData.money >= objsProp.player.tank_hard2.price) {
					buildsGame[ar].num += 1;
				    levelsMaps[select_level].map[0].playerData.money -= objsProp.player.tank_hard2.price;
			    }
			    }
			  }else if(buildsGame[ar].name == "armyFast2") {
		    			if(gameConfig[0].position == "free") {
				if(mapsGame[idMap].playerData.money >= objsProp.player.tank_fast2.price) {
					buildsGame[ar].num += 1;
				    mapsGame[idMap].playerData.money -= objsProp.player.tank_fast2.price;
			    }
			    }else if(gameConfig[0].position == "level") {
			    	if(levelsMaps[select_level].map[0].playerData.money >= objsProp.player.tank_fast2.price) {
					buildsGame[ar].num += 1;
				    levelsMaps[select_level].map[0].playerData.money -= objsProp.player.tank_fast2.price;
			    }
			    }
			  }
		    }
		    ...
		    if(delBuilding.open == true) {
		    	for(let e = 1; e < buildsGame.length; e++) {
		    		if(delBuilding.x == buildsGame[e].x - buildsGame[e].radius - movAddX && delBuilding.y == buildsGame[e].y - buildsGame[e].radius - movAddY) {
		    			if(gameConfig[0].position == "level" && buildsGame[e].health != 0) {
		    				levelsMaps[select_level].map[0].playerData.money += 200;
		    			}else if(buildsGame[e].health != 0){
		    				mapsGame[idMap].playerData.money += 200;
		    			}
		    			buildsGame[e].health = 0;
		    		}
		    	}
		    }
		    		...
		    		for(let r = 0; r < buildsGame.length; r++) {
		    			if(preBuild.empty == true  && stopGame == false) {
		    				...
		    				if(mapsGame[idMap].playerData.money >= buildings[d].price) {
		    				    ...
		    				    }else if(buildings[d].name == "healthReg" || buildings[d].name == "healthReg2") {
