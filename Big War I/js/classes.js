	...
	this.draw = function () {
		if(this.type == "menu") {
			...
			if(this.over == true || this.over == 'true') {
				ctx.globalAlpha = 0.5;
				if(this.played == false && musikPlay == true) {
					butsSong.play();
				}
				this.played = true;
			}else {
...
function build(name, img, x, y, radius, time, map, type, canFaer, reload, ataca, health, addRes) {
    ...
    this.gun2 = new Audio("audio/gun2.mp3");
    this.gun2.loop = false;
	    ...
	    ctx.save();
		    ...
		    if(this.num > 0) {
		        ...
		        if(this.timeOut < this._timeOut && stopGame == false) {
		            ...
		            }else if(this.name == "armyHard2") {
		        		if(gameConfig[0].position == "free") {
		        	        objectsGame.push(new gameObject("tank_hard2", objectImages[6], "player", "dis", this.x + 64, this.y, idMap, objsProp.player.tank_hard2.speed, objsProp.player.tank_hard2.health, objsProp.player.tank_hard2.ataca, objsProp.player.tank_hard2.reload));
		        	    }else {
		        	    	objectsGame.push(new gameObject("tank_hard2", objectImages[6], "player", "dis", this.x + 64, this.y, levelsMaps[select_level].map[0].name, objsProp.player.tank_hard2.speed, objsProp.player.tank_hard2.health, objsProp.player.tank_hard2.ataca, objsProp.player.tank_hard2.reload));
		        	    }
		            }else if(this.name == "armyFast2") {
		        		if(gameConfig[0].position == "free") {
		        	        objectsGame.push(new gameObject("tank_fast2", objectImages[7], "player", "dis", this.x + 64, this.y, idMap, objsProp.player.tank_fast2.speed, objsProp.player.tank_fast2.health, objsProp.player.tank_fast2.ataca, objsProp.player.tank_fast2.reload));
		        	    }else {
		        	    	objectsGame.push(new gameObject("tank_fast2", objectImages[7], "player", "dis", this.x + 64, this.y, levelsMaps[select_level].map[0].name, objsProp.player.tank_fast2.speed, objsProp.player.tank_fast2.health, objsProp.player.tank_fast2.ataca, objsProp.player.tank_fast2.reload));
		        	    }
		            }
