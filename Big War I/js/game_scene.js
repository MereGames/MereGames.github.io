...
var levelsPar = [
    ...
    {level: 5, size: 40, dif: "normal", tum: true, forgets: [{type: "lout", num: 5, end: false}, {type: "enemyKill", num: 560, end: false}, {type: "money", num: 20400, end: false}, {type: "builds", num: 12, end: false}]},
    {level: 6, size: 40, dif: "normal", tum: true, forgets: [{type: "lout", num: 6, end: false}, {type: "enemyKill", num: 600, end: false}, {type: "money", num: 22400, end: false}, {type: "builds", num: 16, end: false}]},
    {level: 7, size: 40, dif: "normal", tum: true, forgets: [{type: "lout", num: 7, end: false}, {type: "enemyKill", num: 450, end: false}, {type: "money", num: 25400, end: false}, {type: "builds", num: 20, end: false}]},
    {level: 8, size: 40, dif: "normal", tum: true, forgets: [{type: "lout", num: 7, end: false}, {type: "enemyKill", num: 450, end: false}, {type: "money", num: 25400, end: false}, {type: "builds", num: 20, end: false}]},
    {level: 9, size: 40, dif: "normal", tum: true, forgets: [{type: "lout", num: 7, end: false}, {type: "enemyKill", num: 450, end: false}, {type: "money", num: 25400, end: false}, {type: "builds", num: 20, end: false}]},
    {level: 10, size: 40, dif: "hard", tum: true, forgets: [{type: "lout", num: 7, end: false}, {type: "enemyKill", num: 450, end: false}, {type: "money", num: 25400, end: false}, {type: "builds", num: 20, end: false}]},
    {level: 11, size: 40, dif: "hard", tum: true, forgets: [{type: "lout", num: 7, end: false}, {type: "enemyKill", num: 450, end: false}, {type: "money", num: 25400, end: false}, {type: "builds", num: 20, end: false}]},
    {level: 12, size: 40, dif: "hard", tum: true, forgets: [{type: "lout", num: 7, end: false}, {type: "enemyKill", num: 450, end: false}, {type: "money", num: 25400, end: false}, {type: "builds", num: 20, end: false}]}
];
...
var delBuilding = {
	...
	draw: function () {
		...
		ctx.drawImage(hamImg, this.x, this.y, 64, 64);
		ctx.restore();
	}
};
...
var sizeMap = levelsPar[select_level].size;
var sizeMissions = levelsPar[select_level].forgets.length;
	...
	if(gameConfig[0].position == "level" || gameConfig[0].endLoad == "level") {
	        ...
	        for(let o = 1; o < viewBorders.length; o++) {
	        	for(let j = 0; j < buildsGame.length; j++) {
	              if(viewBorders[o].view == true) {
	    	            ...
	    	            }else if(buildsGame[j].name == "armyHard2") {
	    	                ctx.drawImage(bordersInfo[5], buildsGame[j].x - buildsGame[j].radius - movAddX, buildsGame[j].y - buildsGame[j].radius - movAddY, 64, 64);
	    	            }else if(buildsGame[j].name == "armyFast2") {
	    	                ctx.drawImage(bordersInfo[6], buildsGame[j].x - buildsGame[j].radius - movAddX, buildsGame[j].y - buildsGame[j].radius - movAddY, 64, 64);
	    	            }
	            }
	        }
	      }
	    ...
	    if(moreEnemy == true) {
	    	if(gameConfig[0].leng == "ru") {
	    		ctx.fillText("Слишком много врагов", WIDTH/2, HEIGHT - 35);
	    	}else if(gameConfig[0].leng == "en") {
	    		ctx.fillText("Too many enemies", WIDTH/2, HEIGHT - 35);
	    	}
	    }
	    ...
	    for(let l = 0; l < levelsPar[select_level].forgets.length; l++) {
	    	...
	    	if(levelsPar[select_level].forgets[l].type == "lout") {
	    		...
	    		if(gameConfig[0].leng == "ru") {
	    		    text = "Убейте " + (levelsPar[select_level].forgets[l].num.toLocaleString() - numKillBoss) + " боссов";
	    	    }else {
	    	    	text = "Kill " + (levelsPar[select_level].forgets[l].num.toLocaleString() - numKillBoss) + " bosses";
	    	    }
	    	}
		...
		for(let k = 0; k < sizeMap; k++) {
			for(let l = 0; l < sizeMap; l++) {
...
setInterval(function () {
	...
	for(let s = 1; s < buildsGame.length; s++) {
		buildsGame[s].iter();
	}
}, 180);
    	...
    	if(buildsGame[r].animEnd == true) {
    		if(buildsGame[r].name == "factory_1" || buildsGame[r].name == "factory_2") {
			    	if(gameConfig[0].position == "free" && buildsGame[r].addRes.name == "money") {
			    		...
			    		levelsMaps[select_level].map[0].playerData.addMoney -= buildsGame[r].addRes.num;
			    	}
			    }else if(buildsGame[r].name == "healthReg") {
			    	addHealth -= buildsGame[r].addRes.num;
			    }

    		buildsGame.splice(r, 1);
    	}
    }
}

function oneSecUpPlayer() {
	setInterval(function () {
		if(stopGame == false && gameConfig[0].position == "level" || stopGame == false && gameConfig[0].position == "free") {
			//Money
		...
		if(gameConfig[0].position == "free") {
		      if(mapsGame[idMap].playerData.time > 0) {
	          	...
	          	mapsGame[idMap].playerData.time = lastTime + 40;
	          	lastTime = lastTime + 40;
	    	...
	    	if(gameConfig[0].position == "level") {
	          ...
	          }else if(numEnemy < (maxPlayer - 23) && numEnemy != maxPlayer){
	          	levelsMaps[select_level].map[0].playerData.time = lastTime + 40;
	          	lastTime = lastTime + 40;
...
function moveMap(pos) {
	...
	if(pos == "left" && movAddX > -TILE_SIZE) {
		movAddX -= SPEED_MAP;
	}else if(pos == "right" && movAddX < (sizeMap * TILE_SIZE + TILE_SIZE - WIDTH)) {
		movAddX += SPEED_MAP;
	}else if(pos == "down" && movAddY < (sizeMap * TILE_SIZE + TILE_SIZE - HEIGHT)) {
		movAddY += SPEED_MAP;
		...
		for(let j = 0; j < mapsGame[idMap].map.length; j++) {
			if(TILE_SIZE*i == objectsGame[u].x - objectsGame[u].radius && TILE_SIZE*j == objectsGame[u].y - objectsGame[u].radius) {
				if(mapsGame[idMap].map[i][j].img == 9 || mapsGame[idMap].map[i][j].img == 8 || mapsGame[idMap].map[i][j].img == 3 || mapsGame[idMap].map[i][j].img == 7) {
					if(objectsGame[u].name != "boss1" && objectsGame[u].name != "boss2" && objectsGame[u].name != "boss3") {
...
function checkMissions() {
	for(let m = 0; m < sizeMissions; m++) {
		...
		}else if(levelsPar[select_level].forgets[m].type == "boss" && levelsPar[select_level].forgets[m].end == false) {
			if(numKillBoss >= levelsPar[select_level].forgets[m].num) {
				levelsPar[select_level].forgets[m].end = true;
			}
		}
...
function createEnemy() {
    ...
    for(let l = 0; l < objectsGame.length; l++) {
  	...
  	for(let k = 0; k < levelsMaps[select_level].map[0].playerData.laut * 10; k++) {
	    ...
	    if(select_level == 4 && k == 1 && levelsMaps[select_level].map[0].playerData.laut == 2){
	    	objectsGame.push(new gameObject("boss1", objectImagesEnemy[2], "enemy", "dis", objBazeEnemy.x + 128, objBazeEnemy.y + 128, levelsMaps[select_level].map[0].name, objsProp.enemy.boss1.speed, objsProp.enemy.boss1.health, objsProp.enemy.boss1.ataca, objsProp.enemy.boss1.reload));
	    }
	    numEnemy+=1;
	    for(let l = 0; l < objectsGame.length; l++) {
	    	if(objectsGame[l].type == "enemy") {
